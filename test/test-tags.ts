...
differentFormatDates(
    ...
    function () {
        ...
        assert.is(
            ...
            'Format of tag: "Date" not correct: "2020.12"'
        );
        res = parseTags('[Date "2020.12.xx"]');
        assert.is(res.messages.length, 1);
        assert.is(
            res.messages[0].message,
            'Format of tag: "Date" not correct: "2020.12.xx"'
        );
        res = parseTags('[UTCDate "?.12.??"]');
        assert.is(res.messages.length, 1);
        assert.is(
            res.messages[0].message,
            'Format of tag: "UTCDate" not correct: "?.12.??"'
        );
    }
);
...
timeControlTags('should raise an error for wrong formats', function () {
    ...
    assert.is(
        res.messages[0].message,
        'Format of tag: "TimeControl" not correct: "400+"'
    );
    res = parseTags('[TimeControl "400*"]');
    assert.is(res.messages.length, 1);
    assert.is(
...
readingCustomTags('should understand custom tags', function () {
    ...
    assert.is((res as any).CustomFakeTag, 'Value');
});
