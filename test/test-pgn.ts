...
completeWithFirst(
    'should notice white starting and color switching each move',
    function () {
...
gameWithResult(
    'should ignore additional white space before or after success',
    function () {
        let my_res = parseGame('1. e4    1-0    ');
        assert.is(tag(my_res, 'Result'), '1-0');
    }
...
gameWithResult(
    'should handle variation at the end even for unclear results',
    function () {
...
gameWithComments(
    'should not read additional whitespace when reading comments',
    function () {
        let my_res = parsePgn(
            '  {First  } 1....    e4   {  third  } e5! {    fourth  }   '
        );
        assert.is(my_res.length, 2);
        assert.is(my_res[0].commentAfter, 'third');
        assert.is(my_res[1].commentAfter, 'fourth');
...
gameWithComments(
    'should understand combination of comment, arrows and fields',
    function () {
...
gameWithComments(
    'should understand permutations of comment, arrows and fields',
    function () {
...
gameWithComments(
    'should understand whitespace when adding fields and arrows',
    function () {
...
gameWithComments(
    'should understand many clock annotations in one move only',
    function () {
        let my_res = parsePgn(
...
gameWithComments(
    'should understand mix of clock comments and normal comments',
    function () {
        let my_res = parsePgn(
            'c4 {Start [%clk 0:10:10] comment [%egt 0:10:10] up to end}'
        );
        assert.ok(my_res[0].commentDiag);
        assert.is(my_res[0].commentDiag['clk'], '0:10:10');
        assert.is(my_res[0].commentDiag['egt'], '0:10:10');
...
gameWithComments(
    'should keep commands sprinkled in comments it cannot parse',
    function () {
        let my_res = parsePgn('1. e4 { [%foo 1.0] } { [%bar any,string] }');
        assert.is(my_res.length, 1);
        let commentDiag = <object>my_res[0].commentDiag;
...
parsingDifferentNotations(
    'should read variants of LAN (long algebraic notation)',
    function () {
...
complexOrErrors(
    'should check error #36 first example: mix of spaces and dots',
    function () {
        const errFunc = () => {
            parsePgn('1. . .');
        };
...
complexOrErrors(
    'should understand error #309: included escaped doublequote',
    function () {
        let my_res = parseGame(
...
parsingNullMoves('should allow null moves inserted', function () {
