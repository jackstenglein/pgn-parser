...
describe('When working with PGN as string', function () {
    describe('When having read the moves', function () {
        it('should have 16 half-moves read', function () {
            let moves = parsePgn(
                '1. e4 c5 2. Nf3 d6 3. d4 cxd4 4. Nxd4 Nf6 5. Nc3 a6 6. Be3 e6 7. f3 Be7 8. Qd2 Qc7 '
            );
...
describe('Reading PGN game with all kinds of comments', function () {
    ...
    it('should not read additional whitespace when reading comments', function () {
        let my_res = parsePgn(
...
c6 {strange moves}`;
        let my_res = parsePgn(input);
        should(my_res.length).equal(2);
        should(my_res[0].commentAfter).equal(
...
does this work?? } c6`;
    ...
    it('should read action comment with underscore (#362)', function () {
        ...
        should.exist(my_res);
        should(my_res[5].commentDiag['c_effect']).equal(
...
describe('Parsing PGN game with all kinds of variation', function () {
    ...
    it('should read hierarchical variation', function () {
        let my_res = parsePgn(
            '1. e4 e5 (1... c5 2. Nf3 (2. d4? cxd4 3. Qxd4)) 2. Nf3 Nc6'
        );
        should(my_res[1].variations[0][0].notation.notation).equal('c5');
        should(my_res[1].variations[0][1].variations[0][0].notation.notation).equal('d4');
        should(my_res[1].variations[0][1].variations[0][1].notation.notation).equal(
            'cxd4'
        );
    });
});
        ...
        assert.throws(
            ...
            (err: any) => {
                ...
                should(err.message).equal(
                    'Expected "O-O", "O-O-O", "x", [RNBQKP], [a-h], or whitespace but "." found.'
                );
                return true;
            }
        );
